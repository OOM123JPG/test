...
class DistributedInferenceEngine:
    ...
    def setup(self):
        ...
        # 流水线并行切分：0-28 层为 Node 0，29-61 层为 Node 1
        mid = 34
        ...
        dist.barrier()
        logger.info(f"Node {self.args.node_rank} Engine Setup Ready.")

    def _pipeline_surgery_and_loading(self):
        """执行 Tucker 替换并把模型移动到 NPU"""
        num_layers = len(self.model.model.layers)
        RESERVED, main_npu = 8, "npu:0" # 留出 14GB 显存给 KV Cache 和临时计算
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            list(executor.map(lambda i: surgery_task(i, self.model.model.layers[i]), range(num_layers)))
        ...
        for i in range(num_layers):
            ...
            if not success: logger.error(f"Layer {self.start_idx+i} Failed to load!")
